...
// ─── 여기서부터 MouthMap 유니폼 선언 ───
float _UseMouthMap; // ToggleUI 속성과 매칭
TEXTURE2D(_MouthMap);                      // Properties에서 정의한 이름과 정확히 일치
SAMPLER(sampler_MouthMap);                 
float _MouthIndex; // 슬라이스 인덱스
// ────────────────────────────────────────
...
half4 GetFinalBaseColor(Varyings input)
{
    ...
    if(_UseMouthMap != 0)
    {
        ...
        half4 mouthColor = _MouthMap.Sample(sampler_MouthMap, mouthUV);
